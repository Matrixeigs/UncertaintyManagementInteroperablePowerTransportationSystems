...
"""
from scipy import inf, ones, eye, diag, array
...
class TwoStageRobustOptimization():
    ...
    def main(self, c, Aeq=None, beq=None, A=None, b=None, lb=None, ub=None, vtypes=None, d=None, G=None,
        ...
        ny = c.shape[0]
        c_first_stage = vstack([c, ones((1, 1))]).toarray()  # Column wise append
        lb_first_stage = vstack([lb, -bigM * ones((1, 1))]).toarray()
        ub_first_stage = vstack([ub, bigM * ones((1, 1))]).toarray()
        ...
        (yy, obj_first_stage, success_first_stage) = lp(c_first_stage, Aeq=Aeq_first_stage, beq=beq,
                                                        A=A_first_stage, b=b, xmin=lb_first_stage,
        ...
        lb_second_stage = vstack([lb_second_stage, -bigM * ones((nu, 1))]).toarray()
        ub_second_stage = vstack([bigM * ones((nlam, 1)), ones((nu, 1)), bigM * ones((nu, 1))]).toarray()
        vtypes_second_stage = ["c"] * nlam + ["b"] * nu + ["c"] * nu
        c_second_stage = zeros((nlam + nu + nu, 1)).toarray()
        ...
        A_second_stage = vstack([A_second_stage, A_second_stage_temp])
        b_second_stage = vstack([b_second_stage, b_second_stage_temp])
        # 3)
        A_second_stage_temp = zeros((nu, nlam + nu + nu))
        b_second_stage_temp = bigM * ones((nu, 1))
        A_second_stage_temp[:, 0:nlam] = -M.transpose()
        ...
        A_second_stage = vstack([A_second_stage, A_second_stage_temp])
        b_second_stage = vstack([b_second_stage, b_second_stage_temp])
        b_second_stage = b_second_stage.toarray()
        A_second_stage = A_second_stage.tocoo()
        Aeq_second_stage = Aeq_second_stage.tocoo()

        (x, obj_second_stage, success_second_stage) = lp(c_second_stage,
                                                         Aeq=Aeq_second_stage,
                                                         beq=beq_second_stage[:, 0].tolist(),
                                                         A=A_second_stage,
                                                         b=b_second_stage[:, 0].tolist(),
        ...
        while Gap > 10 ** -3:
            ...
            vtypes += ["C"] * nx
            lb_first_stage = vstack([lb_first_stage, -inf * ones((nx, 1))]).toarray()
            ub_first_stage = vstack([ub_first_stage, inf * ones((nx, 1))]).toarray()
            c_first_stage = vstack([c_first_stage, zeros((nx, 1))]).toarray()
            ...
            if success_second_stage == 1:
                ...
                b = vstack([b, M.dot(u) - h]).toarray()

                Aeq_first_stage = hstack([Aeq_first_stage, zeros((Aeq_first_stage.shape[0], nx))])
            else:  # No optimal cut is added
                A_first_stage = hstack([A_first_stage, zeros((A_first_stage.shape[0], nx))])
                A_first_stage = vstack([A_first_stage, hstack([-E, zeros((nlam, 1 + nx * k)), -G])])
                b = vstack([b, M.dot(u) - h]).toarray()

            (yy, obj_first_stage, success_first_stage) = lp(c_first_stage, Aeq=Aeq_first_stage,
                                                            beq=beq[:, 0].tolist(),
                                                            A=A_first_stage,
                                                            b=b[:, 0].tolist(),
                                                            xmin=lb_first_stage,
