"""
Stochastic optimal power flow with multiple microgrids, mobile energy storage systems and PVs
@author: Zhao Tianyang
@e-mail: zhaoty@ntu.edu.sg
@date: 13 Jan 2019
@:keyword: renewable energy sources
Major updates:
1) Integration of renewable energy sources, e.g., PVs
2) Stochastic management

Nomenclature:
only the stochastic of PVs is considered in this
"""
...
class StochasticDynamicOptimalPowerFlowTess():
    ...
    def main(self, power_networks, micro_grids, profile, pv_profile, mess, traffic_networks, ns=100):
        ...
        for i in range(ns):
            for t in range(T):
                db_management.insert_data_ds(table_name="distribution_networks", scenario=i, time=t, nl=self.nl,
                                             nb=self.nb, ng=self.ng,
    ...
    def first_stage_problem_formualtion(self, pns, mgs, mess, tns):
        ...
        _nv_first_stage = ng * 2 + nmg * 2 + nmg * 5
        nv_first_stage = _nv_first_stage * T
        # Formulate the boundaries
        lb = concatenate(
            [tile(concatenate([pg_l, rg_l, pg_mg_l, rg_mg_l, pes_ch_l, pes_dc_l, res_l, ees_l, ies_l]), T)])
        ub = concatenate(
            [tile(concatenate([pg_u, rg_u, pg_mg_u, rg_mg_u, pes_ch_u, pes_dc_u, res_u, ees_u, ies_u]), T)])
        # Objective value
        c = concatenate([tile(concatenate([cg, cr, cg_mg, cr_mg, ces_ch, ces_dc, ces, ces_r, ces_i]), T)])
        # Variable types
        ...
        for t in range(T):
            for j in range(ng):
                A[t * ng + j, t * _nv_first_stage + j] = 1
                A[t * ng + j, t * _nv_first_stage + ng + j] = 1
                b[t * ng + j] = pg_u[j]
        # 2) Pg-Rg>=Pgl
        ...
        for t in range(T):
            for j in range(ng):
                A_temp[t * ng + j, t * _nv_first_stage + j] = -1
                A_temp[t * ng + j, t * _nv_first_stage + ng + j] = 1
                b_temp[t * ng + j] = -pg_l[j]
        A = vstack([A, A_temp])
    ...
    def first_stage_solution_validation(self, sol):
        ...
        Iess = zeros((nmg, T))
        for i in range(T):
            Pg[:, i] = sol[_nv_first_stage * i:_nv_first_stage * i + ng]
            Rg[:, i] = sol[_nv_first_stage * i + ng:_nv_first_stage * i + ng * 2]
            Pg_mg[:, i] = sol[_nv_first_stage * i + ng * 2:_nv_first_stage * i + ng * 2 + nmg]
            Rg_mg[:, i] = sol[_nv_first_stage * i + ng * 2 + nmg:_nv_first_stage * i + ng * 2 + nmg * 2]
            Pess_ch[:, i] = sol[_nv_first_stage * i + ng * 2 + nmg * 2:_nv_first_stage * i + ng * 2 + nmg * 3]
            Pess_dc[:, i] = sol[_nv_first_stage * i + ng * 2 + nmg * 3:_nv_first_stage * i + ng * 2 + nmg * 4]
            Ress[:, i] = sol[_nv_first_stage * i + ng * 2 + nmg * 4:_nv_first_stage * i + ng * 2 + nmg * 5]
            Eess[:, i] = sol[_nv_first_stage * i + ng * 2 + nmg * 5:_nv_first_stage * i + ng * 2 + nmg * 6]
            Iess[:, i] = sol[_nv_first_stage * i + ng * 2 + nmg * 6:_nv_first_stage * i + ng * 2 + nmg * 7]
    ...
    def second_stage_problem_formualtion(self, pns, mgs, mess, tns, profile, index=0, weight=1):
        ...
        for i in range(nmg):
            for t in range(T):
                ...
                for j in range(nmes):
                    Aeq_temp[i * T + t, nv_index_ev[j] + t * self.nb_tra_ele + i] = -1  # Sort by order
                    Aeq_temp[i * T + t,
        ...
        for i in range(T):
            for j in range(ng):
                Ts[i * ng + j, i * _nv_first_stage + j] = -1
                Ts[i * ng + j, i * _nv_first_stage + ng + j] = -1
        ...
        for i in range(T):
            for j in range(ng):
                Ts_temp[i * ng + j, i * _nv_first_stage + j] = 1
                Ts_temp[i * ng + j, i * _nv_first_stage + ng + j] = -1
        ...
        for i in range(T):
            for j in range(nmg):
                Ts_temp[i * nmg + j, i * _nv_first_stage + ng * 2 + j] = -1
                Ts_temp[i * nmg + j, i * _nv_first_stage + ng * 2 + nmg + j] = -1
        ...
        for i in range(T):
            for j in range(nmg):
                Ts_temp[i * nmg + j, i * _nv_first_stage + ng * 2 + nmg * 2 + j] = 1  # Charging
                Ts_temp[i * nmg + j, i * _nv_first_stage + ng * 2 + nmg * 3 + j] = -1  # Dis-charging
                Ts_temp[i * nmg + j, i * _nv_first_stage + ng * 2 + nmg * 4 + j] = -1  # Reserve
