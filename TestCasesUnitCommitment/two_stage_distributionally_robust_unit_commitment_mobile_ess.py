"""
Two stage distributionally game with mobile energy storage systems
@author:Zhao Tianyang
@e-mail: zhaoty@jnu.edu.cn

This code is the combination of two stage N-k unit commitment and mobile energy storage networks
...
class TwoStageDistributionallyRobustUnitCommitment():
    def __init__(self):
        self.name = "Two-stage stochastic unit commitment"

    def problem_formulation_first_stage(self, case, delta=0.50, delta_r=0.03):
        baseMVA, bus, gen, branch, gencost, profile = case["baseMVA"], case["bus"], case["gen"], case["branch"], \
                                                          case[
        ...
        for t in range(T):
            for g in range(ng):
                ...
                if t < min(UP_DURATION[g] - gencost[g, MIN_UP], 0):
                    lb[IG * ng * T + t * ng + g] = 1
                else:
                    lb[IG * ng * T + t * ng + g] = 0
                lb[PG * ng * T + t * ng + g] = 0
                ...
                if t < min(DOWN_DURATION[g] - gencost[g, MIN_UP], 0):
                    ub[IG * ng * T + t * ng + g] = 0
                else:
                    ub[IG * ng * T + t * ng + g] = 1
        ...
        for t in range(T):
            # For the unit
            Aeq_temp[t * nl:(t + 1) * nl, NG * ng * T + T * nb + t * nl:NG * ng * T + T * nb + (t + 1) * nl] = -eye(
                    nl)
    ...
    def problem_formulation_seond_stage(self):
        ...
        for t in range(T):
            ...
            for t in range(T):
                ...
                # For the transmission lines
                G_temp[t * nb:(t + 1) * nb,
                ng * T + nd * T + nb * T + t * nl: ng * T + nd * T + nb * T + (t + 1) * nl] = \
                    -(Cft.transpose()).todense()
...
if __name__ == "__main__":
    ...
    two_stage_unit_commitment = TwoStageDistributionallyRobustUnitCommitment()
    ...
    sol = ro.main(c=model_first_stage["c"], Aeq=model_first_stage["Aeq"], beq=model_first_stage["beq"],
                  ...
                  h=model_second_stage["h"], u_mean=model_uncertainty["u_mean"],
                  u_delta=model_uncertainty["u_delta"],
