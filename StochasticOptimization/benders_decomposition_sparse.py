...
7) csr_type sparse matrix is supported
"""
from Solvers.mixed_integer_programming_gurobi import mixed_integer_linear_programming as milp
from numpy import zeros, ones, inf, array, arange, transpose, concatenate
from scipy.sparse import lil_matrix, hstack, vstack
from copy import deepcopy
from Solvers.benders_dual_sparse import linear_programming as lp_dual
from Solvers.benders_primal_sparse import linear_programming as lp_primal
...
class BendersDecomposition():
    ...
    def main(self, c=None, A=None, b=None, Aeq=None, beq=None, lb=None, ub=None, vtype=None, ps=None, qs=None, Ws=None,
        ...
        model_master["c"] = concatenate([model_first_stage["c"], ps], axis=0)
        if model_master["Aeq"] is not None:
            model_master["Aeq"] = hstack(
                [model_first_stage["Aeq"], lil_matrix((model_first_stage["Aeq"].shape[0], self.N))], format='lil')
        if model_master["A"] is not None:
            model_master["A"] = hstack([model_first_stage["A"], lil_matrix((model_first_stage["A"].shape[0], self.N))],
                                       format='lil')

        if model_master["lb"] is not None:
            model_master["lb"] = concatenate([model_first_stage["lb"], -ones((self.N, 1)) * M], axis=0)
        else:
            model_master["lb"] = -ones((self.N + self.nx_first_stage, 1)) * M

        if model_master["ub"] is not None:
            model_master["ub"] = concatenate([model_first_stage["ub"], ones((self.N, 1)) * M], axis=0)
        else:
        ...
        # for i in range(self.N):
        #     sol_second_stage[i] = sub_problem(model_second_stage[i])
        # sol_second_stage_primal = sub_problem(model_second_stage[0])
        A_cuts = lil_matrix((self.N, self.nx_first_stage + self.N))
        b_cuts = zeros((self.N, 1))
        for i in range(self.N):
            # Solve the dual problem
            A_cuts[i, 0:self.nx_first_stage] = -transpose(
                model_second_stage[i]["Ts"].transpose() * sol_second_stage[i]["x"])
        ...
        Gap = self.gap_calculaiton(Upper, Lower)
        Gap_index = [Gap]
        iter_max = 10000
        iter = 0
        ...
        while iter < iter_max:
            ...
            if model_master["A"] is not None:
                model_master["A"] = vstack([model_master["A"], A_cuts], format='lil')
            else:
                model_master["A"] = A_cuts
            if model_master["b"] is not None:
                model_master["b"] = concatenate([model_master["b"], b_cuts], axis=0)
            else:
            ...
            with Pool(n_processors) as p:
                sol_second_stage = list(p.map(sub_problem_dual, model_second_stage))
            # for i in range(self.N):
            #     sol_second_stage[i] = sub_problem_dual(model_second_stage[i])
            ...
            try:
                Upper = min(Upper,
                            transpose(x_first_stage).dot(model_first_stage["c"]) + transpose(objvalue_second_stage).dot(
                                ps))[0][0]
            except:
                try:
                    Upper = min(Upper, transpose(x_first_stage).dot(model_first_stage["c"]) + transpose(
                        objvalue_second_stage).dot(ps))[0]
                except:
                    Upper = min(Upper, transpose(x_first_stage).dot(model_first_stage["c"]) + transpose(
                        objvalue_second_stage).dot(ps))
            print("The upper bound is {0}".format(Upper))

            Gap_index.append(BendersDecomposition.gap_calculaiton(self, Upper, Lower))
            print("The gap is {0}".format(Gap_index[-1]))
            print("The lower bound is {0}".format(Lower))
            iter += 1

            if Gap_index[-1] < eps:
                break
        print("The main process ends! Recovery the results!")
        with Pool(n_processors) as p:
        ...
        for i in range(self.N):
            x_second_stage[i] = array(sol_second_stage[i]["x"]).reshape(self.nx_second_stage, 1)
    ...
    def master_problem(self, model):
        ...
        :return:
        """
        (x, objvalue, status) = milp(model["c"], Aeq=model["Aeq"], beq=model["beq"], A=model["A"], b=model["b"],
    ...
    def sub_problems_update(self, model, x):
        ...
        """
        for i in range(self.N):
            model[i]["b"] = model[i]["hs"] - model[i]["Ts"] * x
...
def sub_problem(model):
    ...
    :return:
    """
    (x, objvalue, status) = lp_primal(model["c"], A=-model["A"], b=-model["b"])
...
def sub_problem_dual(model):
    ...
    :return:
        """
    (x, objvalue, status) = lp_dual(model["b"], Aeq=model["A"].transpose(), beq=model["c"],
                                    xmin=zeros((len(model["b"]), 1)))
...
if __name__ == "__main__":
    ...
    Ts = lil_matrix([[1, 2, -1, 0], [2, -1, 0, -1]])
    hs = array([3, 4]).reshape(2, 1)
    Ws = lil_matrix([1, 3]).reshape(2, 1)
    lb = zeros((4, 1))
    ub = ones((4, 1)) * inf
    qs = array([2]).reshape(1, 1)
    ps = array([1]).reshape(1, 1)
    benders_decomposition = BendersDecomposition()
    sol = benders_decomposition.main(c=c, lb=lb, ub=ub, ps=ps, qs=[qs], hs=[hs], Ts=[Ts], Ws=[Ws])
    print(sol)
    ...
    #
    # Ws = [0] * 3
    # Ws[0] = lil_matrix([[1, 0, -1, 0], [0, 1, 0, -1]])
    # Ws[1] = lil_matrix([[1, 0, -1, 0], [0, 1, 0, -1]])
    # Ws[2] = lil_matrix([[1, 0, -1, 0], [0, 1, 0, -1]])
    #
    # Ts = [0] * 3
    # Ts[0] = lil_matrix([[1, 1], [1 / 3, 1]])
    # Ts[1] = lil_matrix([[5 / 2, 1], [2 / 3, 1]])
    # Ts[2] = lil_matrix([[4, 1], [1, 1]])
    #
    # qs = [0] * 3
    # qs[0] = array([1, 1, 0, 0])
    # qs[1] = array([1, 1, 0, 0])
