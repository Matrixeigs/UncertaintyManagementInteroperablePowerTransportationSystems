"""
Resilient energy management for hybrid AC/DC microgrids
uncertianties includs:
1) Load AC and DC
2) PV output
3) Utility grid
"""
...
class ResilientEnergyManagement():
    """
    ...
    def stochastic_optimization(self, mg, ns=100):
        ...
        for i in range(ns):
            ...
            # Update the rhs, as h = h - E*u
            hs[i] = - model_second_stage[i]["h"] + model_second_stage[i]["M"] * (
                us[i, :].reshape((self.nv_uncertainty, 1)))
            qs[i] = model_second_stage[i]["c"]
            Ws[i] = -model_second_stage[i]["G"].toarray()
            Ts[i] = -model_second_stage[i]["E"].toarray()

        ccg_bd = BendersDecomposition()
        sol = ccg_bd.main(c=model_first_stage["c"], A=model_first_stage["A"].toarray(), b=model_first_stage["b"],
                          Aeq=model_first_stage["Aeq"].toarray(), beq=model_first_stage["beq"],
                          lb=model_first_stage["lb"],
                          ub=model_first_stage["ub"], vtype=model_first_stage["vtypes"], ps=ps, qs=qs, Ws=Ws, hs=hs,
                          Ts=Ts)
        sol_first_stage = sol["x_first_stage"]
        sol_second_stage = sol["x_second_stage"]
        sol_first_stage = self.first_stage_solution_validation(sol=sol_first_stage[:, 0])
    ...
    def first_stage_solution_validation(self, sol):
        ...
        for i in range(ng):
            Fuel[i, 0] = sol[_nv_first_stage * T + i]
    ...
    def second_stage_solution_validation(self, sol):
        ...
        for t in range(T):
            for i in range(ng):
                ...
                mg_sol["fuel"][i, t] = sol[t * _nv_second_stage + FUEL0 + i]
...
if __name__ == "__main__":
    ...
    mg = micro_grid
    mg["VOLL"] = 20
    unit_commitment = ResilientEnergyManagement(mg=mg)
    sol = unit_commitment.stochastic_optimization(mg=mg)
