...
from RobustOptimization.two_stage_robust_optimization import TwoStageRobustOptimization as RO
...
class ResilientEnergyManagement():
    ...
    def robust_optimization(self, mg, ns=1):
        ...
        # Test the two-stage ro problem
        ro = RO()
        # u_mean += u_delta
        sol = ro.main(model_first_stage["c"], Aeq=model_first_stage["Aeq"].toarray(), beq=model_first_stage["beq"],
                      A=model_first_stage["A"].toarray(), b=model_first_stage["b"],
                      lb=model_first_stage["lb"], ub=model_first_stage["ub"],
                      vtypes=model_first_stage["vtypes"], d=model_second_stage[0]["c"],
                      G=-model_second_stage[0]["G"].toarray(), E=-model_second_stage[0]["E"].toarray(),
                      M=-model_second_stage[0]["M"].toarray(), h=-model_second_stage[0]["h"],
                      u_mean=u_mean.reshape((self.nv_uncertainty, 1)),
                      u_delta=u_delta.reshape((self.nv_uncertainty, 1)), budget=array([[u_delta.shape[0] * 1]]), )
        sol = sol[:, 0]
...
if __name__ == "__main__":
    ...
    unit_commitment.robust_optimization(mg=mg)
