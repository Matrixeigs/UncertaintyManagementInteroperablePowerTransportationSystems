...
from StochasticOptimization.benders_decomposition_sparse import BendersDecomposition
...
class UnitCommitment():
    ...
    def stochastic_optimization(self, mg, ns=400):
        ...
        for i in range(ns):
            ...
            hs[i] = - model_second_stage[i]["h"] + model_second_stage[i]["M"]*(us[i, :].reshape((self.nv_uncertainty, 1)))
            qs[i] = model_second_stage[i]["c"]
            Ws[i] = -model_second_stage[i]["G"]
            Ts[i] = -model_second_stage[i]["E"]

        ccg_bd = BendersDecomposition()
        sol = ccg_bd.main(c=model_first_stage["c"], A=model_first_stage["A"], b=model_first_stage["b"],
                          Aeq=model_first_stage["Aeq"], beq=model_first_stage["beq"], lb=model_first_stage["lb"],
                          ub=model_first_stage["ub"], vtype=model_first_stage["vtypes"], ps=ps, qs=qs, Ws=Ws,hs=hs, Ts=Ts)
    ...
    def second_stage_problem_formulation(self, mg, u):
        ...
        # T = vstack([Tx, Ts, Teq, -Teq])  # First stage
        # E = vstack([E, Eeq, -Eeq])  # Uncertainty factors
        # W = vstack([A, Ws, Aeq, -Aeq])  # Second stage
        # h = concatenate([b, hs, beq, -beq])
